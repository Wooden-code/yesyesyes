...
class Level:
    ...
    def load_map_data(self):
        ...
        file_path=r'C:\Users\abc\Desktop\yesyesyes\马里奥游戏\source\data\maps\level_1.json'

        with open(file_path) as f:
            self.map_data = json.load(f)
        try:
            abs = os.getcwd()
            sds = abs.replace('\dist',r'\resource\music\main.ogg')
            # ewr = sds[0] + r'\\resource\\music\\main.ogg'
            pygame.mixer.music.load(sds)  # 方法问题
        except:
        ...
        if 'words' in self.map_data:
            for coin_data in self.map_data['words']:
                ...
                #    self.coin_buff3_group.add(coin.Coin(x, y, type, group))
                    ...
                    self.close_group.add(coin.Coin(x, y, type, group))
    ...
    def update_player_position(self):  # 用当前玩家的速度更新玩家的位置
        ...
        # y direction7
        # self.player.rect.y += self.player.y_vel
        # if self.player.rect.bottom>790:
        #    self.player.rect.bottom=790
    ...
    def check_x_collision(self):  # 果然这里检测的思路统一比较x坐标 传入的时候则不能用最开始使用的传入方法
        ...
        #    self.if_change=True
        # self.adjust_player_x(self.change)

        #    print('oooo')
        # self.background.blit(setup.GRAPHICS['succeed.png'],([i['x'] for i in self.map_data['words']][0],[j['y'] for j in self.map_data['words']][0]))
    ...
    def update_game_window(self):
        ...
        # if self.player.rect.centery<360:

        # print(self.game_window.x,"    ",self.game_window.y)
        # print(self.player.rect.y," pppp")

        # self.y_s+=self.player.y_vel
    ...
    def draw(self, surface):
        ...
        self.close_group.draw(self.game_ground)
       ...
       # self.change_group.draw(self.game_ground)
        ...
        if self.player.rect.x > 15200 and self.game_info['num'] >= 46:
        ...
        # surface.blit(setup.GRAPHICS['remind.png'], (self.player.rect.x, self.player.rect.y))
        if self.change:
            # print('yesyes')
            self.if_change = True
            # surface.blit(setup.GRAPHICS['remind.png'],
            #                ([i['x'] for i in self.map_data['words']][0], [j['y'] for j in self.map_data['words']][0]-50))
            # print([i['group'] for i in self.map_data['words']][0]==4)
        else:
            self.if_change = False
